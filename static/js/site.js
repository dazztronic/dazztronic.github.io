...
function formatSearchResultItem(item, terms) {
  ...
  const relativeUrl = item.ref.replace(/^https?:\/\/[^\/]+/, '');
  
  return (
    `<article class='box'>` +
    `<h1 class='title'>` +
    `<a class='link' class='link' href='${relativeUrl}'>${item.doc.title}</a>` +
    `</h1>` +
...
function documentReadyCallback() {
  ...
  const homepageSearch = document.getElementById("search-homepage");
  const homepageSearchButton = document.getElementById("search-button-homepage");
  
  if (homepageSearch) {
    // Open modal when typing in homepage search
    homepageSearch.addEventListener("keyup", (evt) => {
      if (evt.target.value.trim().length > 0) {
        // Open search modal
        document.querySelector("html").classList.add("is-clipped");
        document.getElementById("search-modal").classList.add("is-active");
        
        // Copy search term to modal
        document.getElementById("search").value = evt.target.value;
        document.getElementById("search").focus();
    ...
    homepageSearchButton.addEventListener("click", (evt) => {
      evt.preventDefault();
      const searchTerm = homepageSearch.value.trim();
